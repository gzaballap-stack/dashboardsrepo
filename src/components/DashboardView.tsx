...
const NAV: { view: View; label: string; group?: string }[] = [
  ...
  { view: "lift_tracker",     label: "Lifting Tracker",   group: "Tools"       },
];
...
export default function DashboardView() {
  ...
  // means no restriction, which is admins and every pre-existing account.
  const [me, setMe] = useState<{ is_admin: boolean; allowed_views: string[] | null } | null>(null);
  ...
  useEffect(() => {
    fetch("/api/me")
      ...
      .then(d => { if (d) setMe({ is_admin: !!d.is_admin, allowed_views: d.allowed_views ?? null }); })
      .catch(() => {});
  }, []);
  ...
  const allGroups = ["Overview", "Raw Data", "Heat Maps", "Agent Stats", "Admin"];

  // ── Feature access ──
  // Until /api/me answers, `me` is null and nothing is filtered — the menu is
  ...
  // in localStorage — move them to the first thing they can actually open.
  useEffect(() => {
    if (!allowedViews) return;
    if (topSection === "tools") {
      if (!toolsVisible.length) { setTopSection("clients_dashboard"); return; }
      if (!toolsVisible.includes(view)) setView(toolsVisible[0] as View);
      return;
    }
    if (topSection === "clients_dashboard") {
      if (!groups.length) {
        const fallback = toolsVisible[0] ?? null;
        if (fallback) { setTopSection("tools"); setView(fallback as View); }
    ...
    <div className="h-[100dvh] overflow-hidden flex" style={{ background: "transparent", position: "relative" }}>
      ...
      `} style={{ width: navCollapsed ? 64 : 240, overflowX: "hidden", transition: "width 320ms cubic-bezier(0.22, 1, 0.36, 1), box-shadow 320ms ease", boxShadow: navCollapsed ? "0 0 0 rgba(0,0,0,0)" : "0 0 40px -8px rgba(0,0,0,0.18)", background: "rgba(255,255,255,0.82)", backdropFilter: "saturate(180%) blur(20px)", WebkitBackdropFilter: "saturate(180%) blur(20px)", borderRight: "1px solid rgba(0,0,0,0.07)" }}>
        ...
        <nav className={`flex-1 overflow-y-auto py-4 ${navCollapsed ? "px-2" : "px-3"}`}>
          ...
          {/* Top-level sections */}
          {TOP_SECTIONS.filter(s => sectionVisible(s.id)).map(sec => {
            ...
            return (
              <div key={sec.id} className="mb-1">
                  ...
                  <div className="mt-1 mb-2" style={{ borderLeft: "1px solid rgba(0,0,0,0.081)", marginLeft: 20, paddingLeft: 8 }}>
                    {groups.map(group => (
                      <div key={group} className="mb-3">
                        ...
                        {NAV.filter(n => n.group === group && can(n.view)).map(item => {
                ...
                {!navCollapsed && sec.id === "tools" && expandedSections.has("tools") && (
                  <div className="mt-1 mb-2" style={{ borderLeft: "1px solid rgba(0,0,0,0.081)", marginLeft: 20, paddingLeft: 8 }}>
                    {NAV.filter(n => n.group === "Tools" && can(n.view)).map(item => {
      ...
      <div className="flex-1 min-w-0 flex flex-col" style={{ position: "relative", zIndex: 1 }}>
        ...
        <main className={`flex-1 overflow-auto ${view === "zip_tool" && topSection === "tools" ? "p-0 flex flex-col" : "p-4 sm:p-6 md:p-8"}`} style={{ background: "transparent" }}>
          ...
          {view === "lift_tracker" && topSection === "tools" && <LiftTracker />}
